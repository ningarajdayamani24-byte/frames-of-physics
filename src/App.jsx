...
function PhysicsCanvas() {
  ...
  useEffect(() => {
    const canvas=canvasRef.current,ctx=canvas.getContext('2d'); let frame,last=0,time=0;
    const resize=()=>{const dpr=Math.min(window.devicePixelRatio||1,1.5);canvas.width=canvas.clientWidth*dpr;canvas.height=canvas.clientHeight*dpr;ctx.setTransform(dpr,0,0,dpr,0,0)};
    const draw=now=>{const dt=Math.min((now-last)/1000||0,.033);last=now;time+=dt;const w=canvas.clientWidth,h=canvas.clientHeight,cx=w*.52,cy=h*.57,scale=Math.min(w,h)*.18,angle=-.42+Math.sin(time*.45)*.07,mag=2.25+Math.sin(time*.7)*.06,ex=cx+Math.cos(angle)*scale*mag,ey=cy+Math.sin(angle)*scale*mag;ctx.clearRect(0,0,w,h);ctx.strokeStyle='rgba(255,255,255,.045)';ctx.lineWidth=1;const gap=42;for(let x=((cx%gap)+gap)%gap;x<w;x+=gap){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,h);ctx.stroke()}for(let y=((cy%gap)+gap)%gap;y<h;y+=gap){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke()}ctx.strokeStyle='rgba(255,255,255,.18)';ctx.beginPath();ctx.moveTo(0,cy);ctx.lineTo(w,cy);ctx.stroke();ctx.beginPath();ctx.moveTo(cx,0);ctx.lineTo(cx,h);ctx.stroke();if(showComponents){ctx.setLineDash([5,6]);ctx.strokeStyle='rgba(125,255,189,.5)';ctx.beginPath();ctx.moveTo(ex,ey);ctx.lineTo(ex,cy);ctx.lineTo(cx,cy);ctx.stroke();ctx.setLineDash([])}ctx.strokeStyle='#8be9ff';ctx.lineWidth=2.2;ctx.shadowColor='rgba(139,233,255,.35)';ctx.shadowBlur=14;ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(ex,ey);ctx.stroke();ctx.shadowBlur=0;const a=Math.atan2(ey-cy,ex-cx),head=12;ctx.fillStyle='#8be9ff';ctx.beginPath();ctx.moveTo(ex,ey);ctx.lineTo(ex-Math.cos(a-.5)*head,ey-Math.sin(a-.5)*head);ctx.lineTo(ex-Math.cos(a+.5)*head,ey-Math.sin(a+.5)*head);ctx.closePath();ctx.fill();ctx.font='12px JetBrains Mono, monospace';ctx.fillStyle='rgba(255,255,255,.5)';ctx.fillText('y',cx+8,18);ctx.fillText('x',w-18,cy-8);ctx.fillStyle='#8be9ff';ctx.fillText('A',ex+12,ey-8);ctx.fillStyle='rgba(255,255,255,.42)';ctx.fillText('θ',cx+42,cy-8);frame=requestAnimationFrame(draw)};
  ...
  return <div className="site">
    ...
    <main id="top">
      <section className="hero section-pad">
        <SimulationBackdrop/><div className="hero-film-vignette"/><div className="hero-orbit orbit-one"/><div className="hero-orbit orbit-two"/>
        <div className="equation-cloud">{equations.map((eq,i)=><span key={i} style={{'--i':i}}>{eq}</span>)}</div>
